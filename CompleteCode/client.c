...
int main(int argc, char const *argv[])
{
    ...
    if(argc == 2)
    {
        ...
        num_char = getline(&buff,&buff_size,file);
        comm_temp = strtok(buff," ");
        //strcpy(command,comm_temp);
        //printf("Command: %s\n",comm_temp);
        /*int len=strlen(command);
        if(messagebody[len-1]=='\n')
        {
            messagebody[len-1]='\0';
        }*/

        if(comm_temp != NULL && !strcmp(comm_temp,"SEND"))
        {
            //mess_temp=strtok(messagebody, "\n");
            strcpy(command,comm_temp);
            mess_temp=strtok(NULL, "\n");
            strcpy(messagebody,mess_temp);
            //printf("Message: %s\n", messagebody);
            /*len=strlen(messagebody);
            if(messagebody[len-1]=='\n')
            {
                messagebody[len-1]='\0';
            }*/
    ...
    while( num_char != EOF || command!=NULL)
    {
        //printf("%s\n",command);
        if(!strcmp(command,SEND))
        {
